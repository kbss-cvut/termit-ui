...
describe("Reducers", () => {
    ...
    describe("messages", () => {
        it("adds message into message array on publish message action", () => {
            ...
            expect(
                reducers(stateToPlainObject(initialState), actionTwo)
            ).toEqual(
    ...
    describe("logout", () => {
        it("resets current user to empty user", () => {
            ...
            expect(
                reducers(stateToPlainObject(initialState), userLogout())
            ).toEqual(Object.assign({}, initialState, { user: EMPTY_USER }));
        });
    ...
    it("does not change loading status on request action with ignoreLoading specified", () => {
        ...
        expect(
            reducers(stateToPlainObject(initialState), action).loading
    ...
    describe("pendingActions", () => {
        ...
        it("does nothing when the same async action request is registered multiple times", () => {
            ...
            expect(
                Object.is(
                    reducers(stateToPlainObject(initialState), action)
                        .pendingActions,
    ...
    describe("errors", () => {
        it("records error with timestamp", () => {
            ...
            expect(result[0].error).toEqual(
                new ErrorInfo(ActionType.LOGIN, error)
            );
        });
    ...
    describe("lastModified", () => {
        it("sets last modified value on UpdateLastModified action", () => {
            ...
            expect(result.lastModified[VocabularyUtils.VOCABULARY]).toEqual(
                value
            );
        });
    });
    ...
    describe("routeTransitionPayload", () => {
        ...
        it("removes payload for specified route name from store on popRoutingPayload action", () => {
            initialState.routeTransitionPayload[Routes.annotateFile.name] =
                "test";
            const result = reducers(
                stateToPlainObject(initialState),
