...
describe("Reducers", () => {
    ...
    describe("fileContent", () => {
        ...
        it("resets file  content on CLEAR_FILE_CONTENT action", () => {
            initialState.fileContent = "test file content";
            const result = reducers(stateToPlainObject(initialState), clearFileContent());
            expect(result.fileContent).toBeNull();
        });
    });
    ...
    describe("routeTransitionPayload", () => {
        it("adds payload to store for specified route name on pushRoutingPayload action", () => {
            const payload = "test";
            const result = reducers(stateToPlainObject(initialState), pushRoutingPayload(Routes.annotateFile, payload));
            expect(result.routeTransitionPayload[Routes.annotateFile.name]).toEqual(payload);
        });
        ...
        it("removes payload for specified route name from store on popRoutingPayload action", () => {
            ...
            const result = reducers(stateToPlainObject(initialState), popRoutingPayload(Routes.annotateFile));
            expect(result.routeTransitionPayload[Routes.annotateFile.name]).not.toBeDefined();
        });
    });
    ...
    describe("desktopView", () => {
        it("changes desktopView to !desktopView on isDesktopView", () => {
            let result = reducers(stateToPlainObject({...initialState, desktopView: true}), changeView());
