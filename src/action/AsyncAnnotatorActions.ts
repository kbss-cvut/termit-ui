...
import {isActionRequestPending, loadTermByIri as loadTermByIriBase} from "./AsyncActions";

export function loadAllTerms(vocabularyIri: IRI, includeImported: boolean = false) {
    const action = {
...
export function loadTermByIri(termIri: string) {
    ...
    return (dispatch: ThunkDispatch, getState: GetStoreState) => {
        ...
        const promise = dispatch(loadTermByIriBase(VocabularyUtils.create(termIri)));
