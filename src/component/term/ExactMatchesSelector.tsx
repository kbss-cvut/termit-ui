...
function filterOutTermsFromCurrentVocabulary(terms: Term[], currentVocabularyIri: string) {
    const result = terms
        .filter(t => t.vocabulary!.iri !== currentVocabularyIri);
    result
        .filter(t => t.plainSubTerms)
        .forEach(t =>
            t.plainSubTerms = t.subTerms!
                .filter( (st) => st.vocabulary!.iri !== currentVocabularyIri )
                .map( st => st.iri )
        );
    return result;
}
    ...
    public fetchOptions = (
        ...
        return this.props
            .loadTerms(
                ...
                VocabularyUtils.create(fetchOptions.option
