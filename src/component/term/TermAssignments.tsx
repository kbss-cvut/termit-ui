...
type TermAssignmentsProps = TermAssignmentsOwnProps &
  HasI18n &
  StoreDispatchProps;
...
function isSuggestedOccurrence(item: AssignmentInfo) {
  return (
    isOccurrence(item) &&
    Utils.sanitizeArray(item.types).indexOf(
      VocabularyUtils.SUGGESTED_TERM_OCCURRENCE
    ) !== -1
  );
}

export class TermAssignments extends React.Component<
  TermAssignmentsProps,
  TermAssignmentsState
> {
  ...
  private loadAssignments() {
    this.props
      .loadTermAssignments(this.props.term)
    ...
    if (resources.length === 0) {
      return (
        <div className="additional-metadata-container italics">
          {i18n("term.metadata.assignments.empty")}
        </div>
      );
    }
    return (
