...
export class BasicTermMetadata extends React.Component<BasicTermMetadataProps, any> {
    ...
    public render() {
        ...
        return (
            <>
                ...
                <Row>
                    <Col xl={2} md={4}>
                        <Label className="attribute-label" title={i18n("term.metadata.vocabulary.tooltip")}>
    ...
    private renderTypes() {
        ...
        return BasicTermMetadata.renderItems(
            Utils.sanitizeArray(types).filter(
                t => t !== VocabularyUtils.TERM && !t.startsWith(SKOS.namespace) && !t.startsWith(OWL.namespace)
            ),
            "term-metadata-types"
        );
    }
    ...
    private static renderItems(items: string[] | string | undefined, containerId?: string) {
        ...
        const renderItem = (item: string) =>
            Utils.isLink(item) ? <OutgoingLink iri={item} label={<AssetLabel iri={item} />} /> : <p>{item}</p>;
