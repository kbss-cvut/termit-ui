...
describe("ParentTermSelector", () => {
  ...
  let loadTermsFromWorkspace: (
    fetchOptions: FetchOptionsFunction
  ) => Promise<Term[]>;
  let loadTermsIncludingCanonical: (
    fetchOptions: FetchOptionsFunction
  ) => Promise<Term[]>;
  ...
  it("loads selector config on mount", () => {
    ...
    expect(StorageUtils.load).toHaveBeenCalledWith(
  ...
  describe("fetchOptions", () => {
    ...
    it("uses vocabulary of term being toggled when loading it subterms", () => {
      ...
      expect((loadTermsFromVocabulary as jest.Mock).mock.calls[0][1]).toEqual(
    ...
    it("passes term being toggled to subterm loading from workspace", () => {
      ...
      wrapper.instance().fetchOptions({ optionID: parent.iri, option: parent });
      expect(
        (loadTermsFromWorkspace as jest.Mock).mock.calls[0][0]
      ).toMatchObject({ optionID: parent.iri });
    });
      ...
      return wrapper
        ...
        .then((options) => {
          ...
          expect(spy).toHaveBeenCalledWith(
            terms,
            [parent.vocabulary!.iri!, vocabularyIri],
            { searchString: "test" }
          );
        });
    });
    ...
    it("passes existing parent terms for inclusion to term loading from vocabulary", () => {
      ...
      wrapper.instance().fetchOptions({});
      expect(
        (loadTermsFromVocabulary as jest.Mock).mock.calls[0][0].includeTerms
      ).toBeDefined();
      expect(
        (loadTermsFromVocabulary as jest.Mock).mock.calls[0][0].includeTerms
      ).toEqual(existingParents.map((p) => p.iri));
    });
