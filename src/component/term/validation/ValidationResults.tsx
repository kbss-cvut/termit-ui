...
const ValidationResults:React.FC<ValidationResultsProps> = props => {
  const {term} = props;
  const validationResults = useSelector((state: TermItState) => state.validationResults[state.vocabulary.iri]);
  ...
  return termResults && termResults.length > 0 ? (
      ...
      >
        {termResults.map((result) => {
          const message = FormValidationResult.fromOntoValidationResult(result, locale);
          return (
              <ValidationMessage key={result.iri} result={message}/>
          );
        })}
      </div>
  ) : (
