...
const ParentTermsList: React.FC<ParentTermsListProps> = props => {
    ...
    return <Row>
        <Col xl={2} md={4}>
            <Label className="attribute-label mb-3">{i18n("term.metadata.parent")}</Label>
        </Col>
        <Col xl={10} md={8}>
            <List type="unstyled" id="term-metadata-parentterms" className="mb-3">
                {parents.map(item => <li key={item.iri}>
                    {workspace.containsVocabulary(item.vocabulary!.iri!) ? <TermLink term={item} language={language}/> :
                        <OutgoingLink label={getLocalized(item.label, language)} iri={item.iri}/>}
                </li>)}
            </List>
        </Col>
    </Row>;
