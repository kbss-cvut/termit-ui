...
describe("Vocabulary Link links to correct internal asset", () => {
    ...
    it("link to an asset", () => {
        ...
        const link = mountWithIntl(<MemoryRouter><VocabularyLink vocabulary={vocabulary}
    ...
    it("links to public vocabulary view when current user is empty", () => {
        const namespace = "http://test.org/";
        const vocabulary = vocGen(namespace);
        const link = mountWithIntl(<MemoryRouter><VocabularyLink vocabulary={vocabulary} user={EMPTY_USER} {...intlFunctions()}/></MemoryRouter>).find(Link);
