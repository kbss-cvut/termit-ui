...
declare type DynamicBreadcrumbRouteProps = DynamicBreadcrumbRouteOwnProps &
  DynamicBreadcrumbRouteStoreProps;
...
const DynamicBreadcrumbRoute = (props: DynamicBreadcrumbRouteProps) => {
  ...
  const renderRoute = (routeProps: RouteComponentProps<any>) => (
    <Breadcrumb
      data={{
        title: props.state[props.asset]
          ? (props.state[props.asset] as Asset).getLabel(locale)
          : "",
...
export default connect<
  DynamicBreadcrumbRouteStoreProps,
  undefined,
  DynamicBreadcrumbRouteOwnProps,
  TermItState
>((state: TermItState) => {
  return { state };
