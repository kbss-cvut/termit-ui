...
export class ResourceSummaryRoute extends React.Component<ResourceSummaryRouteProps> {
  public componentDidMount(): void {
    if (this.props.resource === EMPTY_RESOURCE) {
      this.props.loadResource(
        Utils.extractAssetIri(this.props.match, this.props.location)
      );
    }
  }
  ...
  public componentDidUpdate(): void {
    if (this.props.resource !== EMPTY_RESOURCE) {
      ...
      const routeIri = Utils.extractAssetIri(
        this.props.match,
        this.props.location
      );
      if (
        iri.fragment !== routeIri.fragment ||
        (routeIri.namespace && iri.namespace !== routeIri.namespace)
      ) {
  ...
  public render() {
    ...
    return (
      <>
        <WindowTitle
          title={`${resource.label} | ${this.props.i18n("main.nav.resources")}`}
        />
        {component}
      </>
    );
