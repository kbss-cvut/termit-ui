...
export const DocumentList: React.FC<DocumentListProps> = props => {
    ...
    return <div id="document-list" className="asset-list">
        <Table {...getTableProps()} hover={true} bordered={true} size="sm" >
            ...
            {page.map( (row : Row<Document>) => {
                prepareRow(row);
                return <tr {...row.getRowProps()} onClick={onSelected.bind(this, row.original)}>
                    {row.cells.map(cell =>
        ...
        <Pagination pagingProps={tableInstance as any} pagingState={tableInstance.state as any} allowSizeChange={false}/>
    </div>;
};
