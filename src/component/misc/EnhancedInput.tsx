...
export default class EnhancedInputPropsInput extends AbstractInput<EnhancedInputProps> {
    ...
    public render() {
        const {labelWidth, inputWidth, labelDirection, invalidMessage, help, ...rest} = this.props;

        const input = (
            <React.Fragment>
                <Input type={this.props.type ? this.props.type : "text"} ref={(c: any) => (this.input = c)} {...rest} />
                <FormFeedback>
        ...
        return (
            <FormGroup row={labelDirection === LabelDirection.horizontal}>
                ...
                {labelDirection === LabelDirection.horizontal ? <Col sm={inputWidth}>{input}</Col> : input}
            </FormGroup>
        );
    }
}
