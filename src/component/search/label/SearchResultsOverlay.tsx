...
export const SearchResultsOverlay: React.FC<SearchResultsOverlayProps> = (props: SearchResultsOverlayProps) => {
    ...
    if (props.searchResults.length === 0) {
        ...
        for (i; i < mergedResults.length && i < MAX_RENDERED_RESULTS; i++) {
            ...
            const view = (
                <li key={item.iri} id={`fts-overlay-item-${item.iri}`} className="search-result-link border-bottom">
                    ...
                    <span onClick={props.onClose}>
                        ...
                        {item.vocabulary ? (
                            <span className="small text-muted" style={{marginLeft: "1em"}}>
                                {props.i18n("search.results.vocabulary.from")} <AssetLabel iri={item.vocabulary.iri} />
                            </span>
                        ) : (
                            <span />
                        )}
