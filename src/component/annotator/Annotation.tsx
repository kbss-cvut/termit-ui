...
export class Annotation extends React.Component<AnnotationProps, AnnotationState> {
    ...
    public shouldComponentUpdate(nextProps: Readonly<AnnotationProps>, nextState: Readonly<AnnotationState>, nextContext: any): boolean {
        if (nextProps.sticky !== this.props.sticky || nextProps.text !== this.props.text || nextProps.resource !== this.props.resource || nextProps.score !== this.props.score) {
            return true;
        }
        return this.state.detailOpened !== nextState.detailOpened || this.state.termFetchFinished !== nextState.termFetchFinished || !Asset.equals(this.state.term, nextState.term);
    }
    ...
    private fetchTerm = () => {
        ...
        if (this.props.resource) {
            ...
            this.props.onFetchTerm(this.props.resource).then(t => this.setState({
                term: t,
    ...
    private onSelectTerm = (t: Term | null) => {
        this.setState({
            detailOpened: false
        });
        if (this.props.onUpdate) {
            ...
            newAnnotation.resource = t ? t.iri : this.props.resource;
    ...
    private getTermState = () => {
        ...
        if (this.state.term === null) {
            return isDefinitionAnnotation(this.props.typeof) ? AnnotationClass.PENDING_DEFINITION : AnnotationClass.SUGGESTED_OCCURRENCE;
        }
        if (this.state.term) {
            return isDefinitionAnnotation(this.props.typeof) ? AnnotationClass.DEFINITION : AnnotationClass.ASSIGNED_OCCURRENCE
        }
    ...
    public render() {
        ...
        return <Tag id={id}
            ...
            {this.props.children}
            {this.props.typeof === AnnotationType.DEFINITION ? this.renderDefinitionAnnotation(id) : this.renderOccurrenceAnnotation(id)}
        </Tag>;
    }

    private renderOccurrenceAnnotation(id: string) {
        return <TermOccurrenceAnnotation target={id} term={this.state.term} score={this.props.score}
                                         resource={this.props.resource} text={this.props.text}
                                         annotationClass={this.getTermState()}
                                         annotationOrigin={this.getTermCreatorState()}
                                         isOpen={this.state.detailOpened} onRemove={this.onRemoveAnnotation}
                                         onSelectTerm={this.onSelectTerm} onCreateTerm={this.onCreateTerm}
                                         onToggleDetailOpen={this.toggleOpenDetail} onClose={this.onCloseDetail}/>;
    }

    private renderDefinitionAnnotation(id: string) {
        return <TermDefinitionAnnotation target={id} term={this.state.term} text={this.props.text}
                                         isOpen={this.state.detailOpened}
                                         onRemove={this.onRemoveAnnotation} onSelectTerm={this.onSelectTerm}
                                         onToggleDetailOpen={this.toggleOpenDetail} onClose={this.onCloseDetail}/>;
    }
}


export default connect((state: TermItState, props: AnnotationProps) => {
    return {term: props.resource ? state.annotatorTerms[props.resource] : undefined};
