...
export const TermTypeFrequency: React.FC<TermTypeFrequencyProps> = (props) => {
  ...
  const series = (data as object[]).map((d: any) => ({
    name:
      d["@id"] === TYPE_NOT_FILLED
        ? i18n("statistics.notFilled")
        : d[VocabularyUtils.RDFS_LABEL],
...
export default connect(undefined, (dispatch: ThunkDispatch) => {
  return {
    loadStatistics: (vocabularyIri: string) =>
      dispatch(
        loadStatisticsAction(STATISTICS_TYPE, { vocabulary: vocabularyIri })
      ),
  };
