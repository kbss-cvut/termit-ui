...
function generateControlPanelLink(
  ws: Workspace,
  ...
  return (
    <a
      href={`${url}workspace/${iri.fragment}`}
      title={i18n("workspace.indicator.controlPanelLink.tooltip")}
    >
      {ws.label}
    </a>
  );
...
export const WorkspaceIndicator: React.FC<WorkspaceIndicatorProps> = (
  ...
  const value = Constants.CONTROL_PANEL_URL
    ? generateControlPanelLink(workspace, i18n)
    : workspace.label;
...
}))(injectIntl(withI18n(WorkspaceIndicator)));
