...
export const AssetList: React.FC<AssetListProps> = props => {
    ...
    const renderEmptyInfo = () => !loading ?
        <div className="italics py-2">{i18n("dashboard.widget.assetList.empty")}</div> : null;

    const renderNonEmptyContent = () => {
        const formatter = new TimeAgo(locale);
        return <Table className="widget" borderless={true}>
            <tbody>
            {assets.map(asset => <tr key={asset.iri}>
                        ...
                        <Col xs={12}>
                            <Label className="italics asset-list-title-message"
                                ...
                                {(user.iri === asset.editor!.iri) ?
                                    ...
                                    formatMessage("dashboard.widget.assetList.lastEditMessage", {
    ...
    return <>
        {assets.length > 0 ? renderNonEmptyContent() : renderEmptyInfo()}
    </>;
}

export default connect((state: TermItState) => ({user: state.user}))(AssetList);
